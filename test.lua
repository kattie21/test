...
--     -- Launcher
--     ShowUI                  = true,          -- true = full UI, false = minimal panel
...
--     SaveLogs                = true,          -- write logs to file
--     MaxLogEntries           = 100,           -- max log lines kept
...
--     -- Map Optimization
--     MapOptimization         = true,         -- strip decor/particles/etc
...
--     -- Status API (reconnect_status Rust server)
--     StatusAPIEnabled        = true,           -- send heartbeats to the API
--     StatusAPIURL            = "https://misc.reconnect-tool.tech",  -- reconnect_status server URL
...
local Config = {
    ...
    SaveLogs                = getConfigValue("SaveLogs", true),
    MaxLogEntries           = getConfigValue("MaxLogEntries", 100),
...
local LogoImage = "rbxassetid://91982981915215"
...
local SAVE_KEYS = {
    ...
    "SaveLogs", "HeartbeatEnabled", "ShowFPS",
    "UpdateInterval", "MaxLogEntries",
    "BlackScreen",
    "StatusAPIEnabled", "StatusAPIURL", "StatusAPIKey", "StatusAPIInterval",
}
...
local function writeStatusDirect(status, extra)
    local data = HttpService:JSONEncode({
        version = VERSION, status = status,
        timestamp = os.time(), date = os.date("%Y-%m-%d %H:%M:%S"),
        uptime = os.time() - State.SessionStart,
        uptimeFormatted = formatTime(os.time() - State.SessionStart),
        jobId = game.JobId, placeId = game.PlaceId,
        totalPings = State.TotalPings, failedPings = State.FailedPings,
        reconnects = State.Reconnects, memoryUsage = State.MemoryUsage,
        extra = extra or {}
    })
    if writefile and makefolder and isfolder then
        if not isfolder("ReconnectX") then makefolder("ReconnectX") end
        writefile("ReconnectX/reconnect_status_" .. LP.Name .. ".json", data)
        return true
    end
    return false
end

local function writeStatus(status, extra)
    if not Config.SaveLogs then return true end
    local data = HttpService:JSONEncode({
        version = VERSION, status = status,
        timestamp = os.time(), date = os.date("%Y-%m-%d %H:%M:%S"),
        uptime = os.time() - State.SessionStart,
        uptimeFormatted = formatTime(os.time() - State.SessionStart),
        jobId = game.JobId, placeId = game.PlaceId,
        placeName = pcall(function() return cloneref(game:GetService("MarketplaceService")):GetProductInfo(game.PlaceId).Name end) or "Unknown",
        totalPings = State.TotalPings, failedPings = State.FailedPings,
        reconnects = State.Reconnects, memoryUsage = State.MemoryUsage,
        extra = extra or {}
    })
    return safeWriteFile("reconnect_status_" .. LP.Name .. ".json", data)
end

local function appendLog(status, message)
    if not Config.SaveLogs then return end
    pcall(function()
        local logFile = "reconnect_log_" .. LP.Name .. ".txt"
        local entry = string.format("[%s] [%s] %s", os.date("%Y-%m-%d %H:%M:%S"), status, message or "")
        local existing = safeReadFile(logFile) or ""
        local lines = {}
        for line in existing:gmatch("[^\n]+") do table.insert(lines, line) end
        while #lines >= Config.MaxLogEntries do table.remove(lines, 1) end
        table.insert(lines, entry)
        safeWriteFile(logFile, table.concat(lines, "\n"))
    end)
end
...
local function optimizeMap()
    ...
    local removed = 0
    appendLog("OPTIMIZE", "Starting (" .. level .. ")")
    ...
    appendLog("OPTIMIZE", "Done. " .. removed .. " objects processed")
end
...
local function setupAutoRejoin()
    ...
    pcall(function()
        local conn = TeleportService.TeleportInitFailed:Connect(function(player, result)
            if player == LP then
                appendLog("ERROR", "Teleport failed: " .. tostring(result))
                task.wait(Config.RejoinDelay)
                State.Reconnects = State.Reconnects + 1
                appendLog("REJOIN", "Attempt #" .. State.Reconnects)
...
local function setupConnectionMonitoring()
    local c1 = GuiService.ErrorMessageChanged:Connect(function()
        ...
        if code and code.Value >= Enum.ConnectionError.DisconnectErrors.Value then
            ...
            writeStatusDirect("Disconnected", {errorCode = code.Name})
            appendLog("DISCONNECT", code.Name)
    ...
    local c2 = Players.PlayerRemoving:Connect(function(p)
        if p == LP then
            State.StopUpdate = true; writeStatusDirect("LeftGame"); appendLog("INFO", "Left game")
            if Config.WebhookOnDisconnect then
...
-- ============================================================
-- PHASE 1: STATUS API HEARTBEAT (reconnect_status Rust server)
-- ============================================================
...
local function sendStatusAPIHeartbeat()
    if not Config.StatusAPIEnabled then return false, "disabled" end
    local fn = getHttpFn()
    if not fn then return false, "no_http" end
    local username = LP and LP.Name or "unknown"
    local body = HttpService:JSONEncode({ username = username })
    local ok, resp = pcall(fn, {
        Url     = Config.StatusAPIURL .. "/heartbeat",
        Method  = "POST",
        Headers = {
            ["Content-Type"] = "application/json",
            ["X-API-Key"]    = Config.StatusAPIKey,
        },
        Body = body,
    })
    if not ok then return false, "request_error" end
    if resp and resp.StatusCode == 200 then
        return true
    elseif resp and resp.StatusCode == 429 then
        return false, "rate_limited"
    elseif resp and resp.StatusCode == 401 then
        warn("[Reconnect] Status API: unauthorized - check StatusAPIKey")
        return false, "unauthorized"
    end
    return false, "http_" .. tostring(resp and resp.StatusCode or "nil")
end

local _apiLoopRunning = false

local function startStatusAPILoop()
    if _apiLoopRunning then return end
    if not Config.StatusAPIEnabled then return end
    _apiLoopRunning = true
    task.spawn(function()
        -- health check on first run
        local fn = getHttpFn()
        if fn then
            local ok, resp = pcall(fn, { Url = Config.StatusAPIURL .. "/health", Method = "GET" })
            if ok and resp and resp.StatusCode == 200 then
                appendLog("API", "Status API healthy at " .. Config.StatusAPIURL)
            else
                appendLog("API", "Status API not reachable at " .. Config.StatusAPIURL)
            end
        end
        -- immediate first heartbeat
        local sent, reason = sendStatusAPIHeartbeat()
        if sent then
            appendLog("API", "Heartbeat sent for " .. (LP and LP.Name or "?"))
        elseif reason ~= "disabled" and reason ~= "no_http" then
            appendLog("API", "Heartbeat failed: " .. tostring(reason))
        end
        -- rolling loop
        while _apiLoopRunning and not State.StopUpdate do
            task.wait(Config.StatusAPIInterval)
            if not Config.StatusAPIEnabled then break end
            local s, r = sendStatusAPIHeartbeat()
            if not s and r ~= "disabled" and r ~= "no_http" then
                appendLog("API", "Heartbeat failed: " .. tostring(r))
            end
        end
        _apiLoopRunning = false
    end)
end

local function stopStatusAPILoop()
    _apiLoopRunning = false
end
...
local function iconBox(parent, iconId, bgColor, size, iconSize)
    ...
    createIcon(box, iconId, iSize, Theme.Text)
...
addQuickAction(actionsGrid, {
    Icon = Icons.Star, IconColor = Color3.fromRGB(52, 211, 153),
    Label = "Save Status", Order = 4,
    Callback = function()
        writeStatus("Running")
        notify("Saved", "Status file written", "success", 2)
    end,
})
...
addToggle(generalSection, {
    Label = "Save Logs", Desc = "Write session logs to file",
    Icon = Icons.FileText, IconColor = Theme.Info,
    Default = Config.SaveLogs, Order = 1,
    Callback = function(v) Config.SaveLogs = v; saveSettings() end,
})

addToggle(generalSection, {
    Label = "Heartbeat Monitor", Desc = "Track connection health",
    Icon = Icons.Wifi, IconColor = Theme.Success,
    Default = Config.HeartbeatEnabled, Order = 2,
...
addToggle(generalSection, {
    ...
    Icon = Icons.Monitor, IconColor = Theme.Primary,
    Default = Config.ShowFPS, Order = 3,
...
addDropdown(intervalSection, {
    Label = "Max Log Entries", Options = {"50", "100", "200", "500"},
    Default = tostring(Config.MaxLogEntries), Order = 2,
    Callback = function(v) Config.MaxLogEntries = tonumber(v) or 100; saveSettings() end,
})
...
addButton(dangerSection, {
    Text = "Clear All Logs", Icon = Icons.Trash,
    Variant = "danger", Size = UDim2.new(1, 0, 0, 36), Order = 2,
    Callback = function()
        pcall(function()
            if delfile and isfile then
                local f = "ReconnectX/reconnect_log_" .. LP.Name .. ".txt"
                if isfile(f) then delfile(f) end
            end
        end)
        notify("Cleared", "Log files deleted", "success", 2)
    end,
})

addButton(dangerSection, {
    Text = "Destroy UI", Icon = Icons.Power,
    Variant = "danger", Size = UDim2.new(1, 0, 0, 36), Order = 3,
    Callback = function()
        cleanup()
        ScreenGui:Destroy()
    end,
...
writeStatus("Running")
appendLog("INIT", "Reconnect v" .. VERSION .. " started - " .. LP.DisplayName)
...
task.spawn(function()
    while not State.StopUpdate do
        pcall(function()
            ...
            updateUI()
            writeStatus(State.CurrentStatus)
        end)
